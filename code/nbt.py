...
def track_woz_data(dialogues, model_variables, word_vectors, dialogue_ontology, sessions):
    ...
    #print json.dump(indexed_dialogues, indent=4)
...
def generate_examples(target_slot, feature_vectors, word_vectors, dialogue_ontology,
    ...
    for idx_example, example in enumerate(examples):
        ...
        original_sentence = utterance[0][0]
    ...
    return (original_sentence, features_full, features_requested_slots, features_confirm_slots, \
...
def evaluate_model(dataset_name, sess, model_variables, data, target_slot, utterances, dialogue_ontology, \
    ...
    true_positives, train_step, update_coefficient, h_utterance_representation = model_variables

    (original_sentence, xs_full, xs_sys_req, xs_conf_slots, xs_conf_values, xs_delex, xs_labels, xs_prev_labels) = data
...
def extract_feature_vectors(utterances, word_vectors, ngram_size=3, longest_utterance_length=40, use_asr=False, use_transcription_in_training=False):
    ...
    filename = 'word-vectors/'+nowDatetime+'_w2v_sentences.txt'
    ...
    for idx, utterance in enumerate(utterances):
        ...
        #print "Full ASR is", full_asr[0][0]
        ...
        #print curr_confirm_slots, curr_confirm_values

        
        #write the sentence, state
        f.write(utterances[idx][0][0])
        f.write("\t")
        '''
        curr_state = ''
        if curr_confirm_slots:
            for i in range(len(curr_confirm_slots)):
                temp = curr_confirm_slots[i]
                if temp != '':
                    if temp == 'request':
                        curr_state += temp + ':' + curr_confirm_values[i]+','
                    else:
                        curr_state += 'inform:'+temp+','                    
...
#        print "======", full_asr
        ...
        #print utterances[idx][0][1]
        #print ngram_feature_vectors[idx]
...
def train_run(target_language, override_en_ontology, percentage, model_type, dataset_name, word_vectors, exp_name, dialogue_ontology, model_variables, target_slot, language="en", max_epoch=20, batches_per_epoch=4096, batch_size=256):
    ...
    correct_prediction, true_positives, train_step, update_coefficient, h_utterance_representation = model_variables
    ...
    utterances_train = utterances_train2 # utterances_train2 + utterances_val2[0:int(0.75 * val_count)]
    utterances_val = utterances_val2 # utterances_val2[int(0.75 * val_count):]
    ...
    sess = tf.Session()
    sess.run(init)
    f = open('./word-vectors/w2v_sentences_cnn__.csv', 'w')
    ...
    #train_writer = tf.summary.FileWriter('./logs/train/ori_nbt', sess.graph) #, sess.graph
    sentences_cnn_rep = []
    while epoch < max_epoch:
        ...
        for batch_id in range(batches_per_epoch):
            ...
            (original_sentence, batch_xs_full, batch_sys_req, batch_sys_conf_slots, batch_sys_conf_values, 
                batch_delex, batch_ys, batch_ys_prev) = batch_data
            
            #print 'batch_xs_full shape: ', batch_xs_full.shape # (256, 40, 300)

            #tf.summary.scalar('accuracy', accuracy)
            #tf.summary.scalar('f_score', f_score)
            #tf.summary.scalar('precision', precision)
            #tf.summary.scalar('recall', recall)
            #tf.summary.scalar('cross_entropy', train_step)
            ...
            [_, cf, cp, cr, ca, sv] = sess.run([train_step, f_score, precision, recall, accuracy, h_utterance_representation], feed_dict={x_full: batch_xs_full, \
                                              x_delex: batch_delex, \
            ...
            #train_writer.add_summary(summary, batch_id)
            path_to_save = './logs/train/ori_nbt/'+str(override_en_ontology) + "_" +str(target_slot)+'_model.ckpt'
            saver.save(sess, path_to_save)

            sv_dic = {}
            sv_dic[original_sentence] = sv
            sentences_cnn_rep.append(sv_dic) #numpy.save(f, sv)#'''
        ...
        if current_metric > best_f_score:
            ...
            path_to_save = "./models/origin_nbt/" + model_type + "_" + language + "_" +  str(override_en_ontology) + "_" + \
                   str(dataset_name) + "_" + str(target_slot)+ "_" + str(exp_name) + "_" + str(percentage) + ".ckpt"

            save_path = saver.save(sess, path_to_save)

    numpy.save(f, sentences_cnn_rep) #train_writer.close()
...
class NeuralBeliefTracker:
    ...
    def track_utterance(self, current_utterance, req_slot="", conf_slot="", conf_value="", past_belief_state=None):
        ...
        for slot in self.dialogue_ontology:
            ...
            try:
                path_to_load = "models/origin_nbt/" + self.model_type + "_en_False_" + \
    ...
    def test_woz(self):
        ...
        for model_id in range(0, self.num_models):
            ...
            for load_slot in slots_to_load:
                ...
                path_to_load = "./models/origin_nbt/" + self.model_type + "_" + self.language_suffix + "_" + str(override_en_ontology) + "_" + \
...
def main():
    ...
    if do_training:
        ...
        while True:
            ...
            req_slot = raw_input("Enter system requirement slot:")
