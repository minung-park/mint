...
from models_768 import model_definition
...
def load_word_vectors(file_destination, primary_language="english"):
    ...
    for line in f:
        ...
        if language_key in transformed_key:     
            ...
            if word_dictionary[transformed_key].shape[0] != 768: #300
...
def extract_feature_vectors(utterances, sentence_vectors, word_vectors, ngram_size=3, longest_utterance_length=40, use_asr=False, use_transcription_in_training=False):
    ...
    word level 에서는 기존 nbt 처럼 concat 하여 사용 -> 40*768
    sentence level 에서는 문장을 바로 임베딩하여 사용 -> 1*768
    '''
...
def train_run(target_language, override_en_ontology, percentage, model_type, dataset_name, sentence_vectors, word_vectors, exp_name, dialogue_ontology, model_variables, target_slot, language="en", max_epoch=20, batches_per_epoch=4096, batch_size=256):
    ...
    while epoch < max_epoch:
        ...
        if current_metric > best_f_score:
            ...
            path_to_save = "./models/bert_sentences/" + model_type + "_" + language + "_" +  str(override_en_ontology) + "_" + \
    ...
    def __init__(self, config_filepath):
        ...
        for slot in dialogue_ontology:
            ...
            if slot == "request":
                ...
                slot_vectors = numpy.zeros((len(dialogue_ontology[slot]), 768), dtype="float32")
                value_vectors = numpy.zeros((len(dialogue_ontology[slot]), 768), dtype="float32")
                ...
                slot_vectors = numpy.zeros((len(dialogue_ontology[slot])+1, 768), dtype="float32") # +1 for None
                value_vectors = numpy.zeros((len(dialogue_ontology[slot])+1, 768), dtype="float32")
