...
from models_1024 import model_definition
from bert_serving.client import BertClient

global_var_asr_count = 1
bc = BertClient(ip='192.168.100.13', check_version=False, check_length=False)
...
def xavier_vector(word, D=1024):
    ...
    vector = bc.encode(oov_word)	
    return numpy.reshape(vector[0], (D))


def bert_sentence_embedding(sentence):
    sentences_list = []
    sentences_list.append(sentence)
    vector = bc.encode(sentences_list)
    return vector[0]
...
def evaluate_woz(evaluated_dialogues, dialogue_ontology):
    ...
    """
    print_mode=True
    informable_slots = list(set(["food", "area", "price range", "prezzo", "cibo", "essen", "preisklasse", "gegend", "가격", "음식", "지역"]) & set(dialogue_ontology.keys()))
...
def process_woz_dialogue(woz_dialogue,language,override_en_ontology):
    ...
    if language == "english" or language == "en" or override_en_ontology:
        ...
        null_bs["지역"] = "none"
        null_bs["request"] = []
        informable_slots = ["음식", "가격", "지역"]
        pure_requestables = ["주소", "전화번호", "우편번호"]
...
#                        print "!!!!!", inf_slot, turn[5][inf_slot]
            ...
            #print "past belief state", turn[5]
...
def track_woz_data_each_slot(dialogues, model_variables, sentence_vectors, word_vectors, dialogue_ontology, sess, slot):
    """
    ...
    for idx in range(0, dialogue_count):
        ...
        evaluated_dialogue, belief_states = track_dialogue_woz_each_slot(model_variables, sentence_vectors, word_vectors, dialogue_ontology, dialogues[idx], sess, slot)
...
def track_woz_data(dialogues, model_variables, sentence_vectors, word_vectors, dialogue_ontology, sessions):
    """
    This method evaluates the WOZ dialogues. 
    """
    evaluated_dialogues = []
    ...
    for idx in range(0, dialogue_count):
        ...
        evaluated_dialogue, belief_states = track_dialogue_woz(model_variables, sentence_vectors, word_vectors, dialogue_ontology, dialogues[idx], sessions)
        evaluated_dialogues.append(evaluated_dialogue)
        list_of_belief_states.append(belief_states)
...
def track_dialogue_woz_each_slot(model_variables, sentence_vectors, word_vectors, dialogue_ontology, woz_dialogue, sess, slot):
    """
    ...
    for idx, trans_and_req_and_label_and_currlabel in enumerate(woz_dialogue):
        ...
        if slot == "request":
            updated_belief_state = sliding_window_over_utterance(sess, example, sentence_vectors, word_vectors, dialogue_ontology, mx, slot, print_mode=False)
        ...
        for idx_value, value in enumerate(dialogue_ontology[slot]):
            if slot in "request":
                ...
                current_bs[slot] = print_belief_state_woz_informable(dialogue_ontology[slot], updated_belief_state, threshold=0.01)
...
def track_dialogue_woz(model_variables, sentence_vectors, word_vectors, dialogue_ontology, woz_dialogue, sessions):
    """
    This produces a list of belief states predicted for the given WOZ dialogue. 
    """
    ...
    for idx, trans_and_req_and_label_and_currlabel in enumerate(woz_dialogue):
        ...
        for slot in slots_to_track:
            ...
            if slot == "request":
                ...
                updated_belief_state = sliding_window_over_utterance(sessions[slot], example, sentence_vectors, word_vectors, dialogue_ontology, mx, slot, print_mode=False)
                # updated_belief_state[updated_belief_state < 0.5] = 0
                list_of_belief_states[idx]["request"] = updated_belief_state
                #print updated_belief_state
            ...
            for idx_value, value in enumerate(dialogue_ontology[slot]):
                if slot in "request":
                    current_bs[slot] = print_belief_state_woz_requestables(dialogue_ontology[slot], updated_belief_state, threshold=0.5) #0.5
                else:
        ...
        #print "True State: ", label
        #print "Prediction: ", current_bs
        predictions_for_dialogue.append((trans_plus_sys, {"True State": label}, {"Prediction": current_bs}))
...
def generate_data(utterances, sentence_vectors, word_vectors, dialogue_ontology, target_slot):
    """
    ...
    feature_vectors = extract_feature_vectors(utterances, sentence_vectors, word_vectors)
    ...
    for slot_idx, slot in enumerate(list_of_slots):
            ...
            if slot != "request":
                ...
                for value_idx, value in enumerate(dialogue_ontology[slot]):
                    ...
                    if (slot, value) in utterance[4]: # utterances are ((trans, asr), sys_req_act, sys_conf, labels)
                        ...
                        #print "POS:", utterance_idx, str(utterance), value_idx, value
                    else:
        ...
        #print slot, len(positive_examples)#positive_examples[slot] = set(positive_examples[slot])
...
def generate_examples(target_slot, feature_vectors, word_vectors, dialogue_ontology,
    ...
    '''
    print 'features_requested_slots shape: ', features_requested_slots.shape
    print 'features_confirm_slots shape: ', features_confirm_slots.shape
    print 'features_confirm_values shape: ', features_confirm_values.shape
    print 'features_full shape: ', features_full.shape
    print 'features_delex shape: ', features_delex.shape
    print 'features_previous_state shape: ', features_previous_state.shape
    '''
...
def evaluate_model(dataset_name, sess, model_variables, data, target_slot, utterances, dialogue_ontology, \
    ...
    for idx in range(0, batch_count):
        ...
        if idx in [batch_count - 1, 0]:
            ...
            xss_full = numpy.zeros((batch_size, word_vector_size), dtype="float32") #longest_utterance_length, 
        ...
        #print 'xss_full shape: ', xss_full.shape
        #print 'xs_full shape: ', xs_full.shape
...
def load_sentence_vectors(file_destination):
    """
    ...
    print "Loading BERT sentence vectors from", file_destination
    sentence_dictionary = {}
    ...
    for line in f:
        ...
        sentence = line[0]
        vectors = line[1].replace('\t', ' ')
        sentence_dictionary[sentence] = numpy.fromstring(vectors, dtype="float32", sep=" ")
       
    print len(sentence_dictionary), "vectors loaded from", file_destination     
...
def load_word_vectors(file_destination, primary_language="english"):
    ...
    for line in f:
        ...
        if language_key in transformed_key:     
            ...
            vectors = line[1].replace('\t', ' ')			
            word_dictionary[transformed_key] = numpy.fromstring(vectors, dtype="float32", sep=" ")

            if word_dictionary[transformed_key].shape[0] != 1024: #300
...
def extract_feature_vectors(utterances, sentence_vectors, word_vectors, ngram_size=3, longest_utterance_length=40, use_asr=False, use_transcription_in_training=False):
    """
    ...
    '''
    bert 적용하면서 word/sentence 레벨로 분리하여 feature vector 생성
    word level 에서는 기존 nbt 처럼 concat 하여 사용 -> 40*1024
    sentence level 에서는 문장을 바로 임베딩하여 사용 -> 1*1024
    '''
       
    config = ConfigParser.RawConfigParser()
    home_path = os.path.expanduser('~')
    config_file = home_path + "/origin_neural-belief-tracker/config/woz_stat_update_bert.cfg"
    config.read(config_file)
    
    using_embedding = config.get("data", "using_embedding")

    if using_embedding == 'sentences':
        longest_utterance_length = 1
    ...
    for idx, utterance in enumerate(utterances):
        ...
        for sub_idx in range(0, confirmation_count):
            ...
            if current_cslot != "" and current_cvalue != "":
                ...
                #print current_cslot, ':', current_cvalue			
        ...
        if using_embedding == 'sentences':
            for j in range(0, utterance_count):
                ngram_feature_vectors.append(numpy.zeros((word_vector_size,), dtype="float32"))

            full_fv = numpy.zeros((word_vector_size, ), dtype="float32")
            not_found_sv = []
            for (c_example, asr_coeff) in full_asr[0:asr_count]:
                if c_example != "":
                    c_example = unicode(c_example)
                    if c_example not in sentence_vectors: #sentences vector 리스트에 없을 경우
                        not_found_sv.append(c_example)
                        sv = bc.encode(not_found_sv) #새로 인코딩
                        full_fv = sv[0]
                        #ngram_feature_vectors.append(numpy.reshape(sv, (word_vector_size)))
                        
                    else:
                        full_fv = sentence_vectors[c_example]
                        #ngram_feature_vectors.append(numpy.reshape(sentence_vectors[c_example], (word_vector_size)))
                asr_weighted_feature_vectors.append(full_fv) #numpy.reshape(full_fv, (word_vector_size))
    ...
    #print "ngram_feature_vectors shape: ", ngram_feature_vectors[0].shape
    #print "requested_slot_vectors shape: ", requested_slot_vectors[0].shape
    #print "confirm_slots: ", confirm_slots[0].shape
    #print "confirm_values: ", confirm_values[0].shape
    #print list_of_features[0]

    return list_of_features


def train_run(target_language, override_en_ontology, percentage, model_type, dataset_name, sentence_vectors, word_vectors, exp_name, dialogue_ontology, model_variables, target_slot, language="en", max_epoch=20, batches_per_epoch=4096, batch_size=256):
    """
    ...
    #print utterances_train[0][0]

    # training feature vectors and positive and negative examples list. 
    print "Generating data for training set:"
    feature_vectors, positive_examples, negative_examples = generate_data(utterances_train, sentence_vectors, word_vectors, dialogue_ontology, target_slot)
    ...
    fv_validation, positive_examples_validation, negative_examples_validation = \
                generate_data(utterances_val, sentence_vectors, word_vectors, dialogue_ontology, target_slot)
    ...
    #train_writer = tf.summary.FileWriter('./logs/train/bert_sentence', sess.graph)
    ...
    while epoch < max_epoch:
        ...
        for batch_id in range(batches_per_epoch):
            ...
            #print 'batch_xs_full shape: ', batch_xs_full.shape
        ...
        #path_to_save = './logs/train/bert_sentence/'+str(override_en_ontology) + "_" +str(target_slot)+'_model.ckpt'
        ...
        if current_metric > best_f_score:
            ...
            path_to_save = "./models/bert_large_sentences/" + model_type + "_" + language + "_" +  str(override_en_ontology) + "_" + \
    ...
    '''
    minyoung
    해당 모델로 바로 테스팅하는 코드 추가.....
    target_language, override_en_ontology, percentage, model_type, dataset_name, sentence_vectors, word_vectors, exp_name, dialogue_ontology, model_variables, target_slot, language="en", max_epoch=20, batches_per_epoch=4096, batch_size=256

    test_dialogues, training_turns = load_woz_data("data/" + dataset_name + "/" + dataset_name + "_test_en_1.json", target_language, override_en_ontology=False)
    evaluated_dialogues, belief_states = track_woz_data_each_slot(test_dialogues, model_variables, sentence_vectors, word_vectors, dialogue_ontology, sess, target_slot)
    results = evaluate_woz(evaluated_dialogues, self.dialogue_ontology)
    json.dump(evaluated_dialogues, open("results/"+"_woz_tracking__.json", "w"), indent=4)
...
def sliding_window_over_utterance(sess, utterance, sentence_vectors, word_vectors, dialogue_ontology, model_variables, target_slot, print_mode=True):
    """
    """
    ...
    list_of_outputs = test_utterance(sess, utterance, sentence_vectors, word_vectors, dialogue_ontology, model_variables, target_slot, print_mode)
    belief_state = list_of_outputs[0]

    return belief_state


def test_utterance(sess, utterances, sentence_vectors, word_vectors, dialogue_ontology, model_variables, target_slot, do_print=True):
    """
    ...
    if target_slot == "request":
        ...
        value_count = len(potential_values) + 1 # for None

    # should be a list of features for each ngram supplied. 
    fv_tuples = extract_feature_vectors(utterances, sentence_vectors, word_vectors, use_asr=True)
    utterance_count = len(utterances)
    ...
    for idx_hyp, extracted_fv in enumerate(fv_tuples):
        ...
        #print 'prev belief state ', prev_belief_state
    ...
    distribution, update_coefficient_load = sess.run([y, update_coefficient], feed_dict={x_full: fv_full, 
                                      ...
                                      requested_slots: fv_sys_req, system_act_confirm_slots: fv_conf_slot, system_act_confirm_values: fv_conf_val,\
                                      y_past_state: features_previous_state, keep_prob: 1.0}) #1.0
    ...
    if do_print:
        print 'user: ', utterances[idx_hyp][0][0]	
        print_slot_predictions(list_of_belief_states[0], potential_values, target_slot, threshold=0.01)
...
class NeuralBeliefTracker:
    ...
    def __init__(self, config_filepath):
        ...
        sentence_vectors = {}
        sentence_vector_destination = config.get("data", "sentence_vectors")
        ...
        sentence_vectors = load_sentence_vectors(sentence_vector_destination)
        ...
        if language == "korean":
            word_vectors["postcode"] = word_vectors[u"우편번호"] #+ word_vectors["번호"]
            word_vectors["request"] = word_vectors[u"요청"]
            word_vectors["price range"] = word_vectors[u"가격"]
            word_vectors["dontcare"] = word_vectors[u"무신경"] #+ word_vectors["안"] + word_vectors["씀"]
            word_vectors[u"보통의"] = word_vectors[u"보통"]
        ...
        for slot in dialogue_ontology:
            ...
            if slot == "request":
                ...
                slot_vectors = numpy.zeros((len(dialogue_ontology[slot]), 1024), dtype="float32")
                value_vectors = numpy.zeros((len(dialogue_ontology[slot]), 1024), dtype="float32")
                ...
                self.model_variables[slot] = model_definition(word_vector_size, len(dialogue_ontology[slot]), slot_vectors, value_vectors, longest_utterance_length=self.longest_utterance_length, \
                ...
                slot_vectors = numpy.zeros((len(dialogue_ontology[slot])+1, 1024), dtype="float32") # +1 for None
                value_vectors = numpy.zeros((len(dialogue_ontology[slot])+1, 1024), dtype="float32")
                ...
                self.model_variables[slot] = model_definition(word_vector_size, len(dialogue_ontology[slot]), slot_vectors, value_vectors, longest_utterance_length=self.longest_utterance_length, \
                                                              use_delex_features=self.use_delex_features, use_softmax=True, value_specific_decoder=self.value_specific_decoder, learn_belief_state_update=self.learn_belief_state_update)
                
            '''
            keep_prob, x_full, x_delex, requested_slots, system_act_confirm_slots, system_act_confirm_values, \
            y_, y_past_state, accuracy, f_score, precision, recall, num_true_positives, \
            num_positives, classified_positives, y, predictions, true_predictions,  \
            correct_prediction, true_positives, train_step, update_coefficient = self.model_variables[slot]
            
            print 'NeuralBeliefTracker init ======= x_full shape: ', x_full.shape
            '''
    ...
    def track_utterance(self, current_utterance, req_slot="", conf_slot="", conf_value="", past_belief_state=None):
        ...
        for slot in self.dialogue_ontology:
            ...
            try:
                path_to_load = "models/bert_sentences/" + self.model_type + "_en_False_" + \
            ...
            belief_state = sliding_window_over_utterance(sess, utterance, self.sentence_vectors, self.word_vectors, self.dialogue_ontology, self.model_variables, slot)
    ...
    def train(self):
        ...
        for slot in self.dialogue_ontology.keys():
            ...
            train_run(target_language=self.language, override_en_ontology=False, percentage=1.0, model_type="CNN", dataset_name=self.dataset_name, sentence_vectors = self.sentence_vectors,\
    ...
    def test_woz(self):
        ...
        #woz_dialogues, training_turns = load_woz_data("data/" + self.dataset_name + "/" + self.dataset_name + "_validate_" + self.language_suffix + ".json", self.language, override_en_ontology=False)
        sessions = {}
        saver = tf.train.Saver()
        ...
        for model_id in range(0, self.num_models):
            ...
            elif self.language == "korean" or self.language == "ko":
                slots_to_load = ["food", "price range", "area", "request"]#[u"음식", u"가격", u"지역", "request"]

            for load_slot in slots_to_load:

                path_to_load = "./models/bert_sentences/" + self.model_type + "_" + self.language_suffix + "_" + str(override_en_ontology) + "_" + \
            ...
            evaluated_dialogues, belief_states = track_woz_data(woz_dialogues, self.model_variables, self.sentence_vectors, self.word_vectors, self.dialogue_ontology, sessions)
...
def main():
    ...
    try: 
        ...
        #home_path = os.path.expanduser('~')
        #config_filepath = home_path + "/origin_neural-belief-tracker/config/woz_stat_update_bert.cfg"
    except:
